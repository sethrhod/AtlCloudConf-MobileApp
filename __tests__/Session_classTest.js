import Session from "../components/scripts/Session_class";
...
test("Session class constructor", () => {
  const session = new Session(data[0].sessions[0], speaker_data);
  expect(session.id).toBe("b5d96beb-0c81-491a-86ee-ed98f061b524");
  expect(session.title).toBe("Check-in");
  expect(session.description).toBe(null);
  expect(session.startsAt).toBe("2023-03-25T07:45:00");
  expect(session.endsAt).toBe("2023-03-25T08:45:00");
  expect(session.room).toBe("Building Q Lobby");
  expect(session.speakers_small_object).toEqual([]);
